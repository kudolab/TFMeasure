...
def LSTF(num, averaging_times, trim_imp_start, trim_imp_end, trim_inv_start, trim_inv_end, outdir):
    ...
    trim(outdir+"/inv_cLSTF_"+str(num)+".DDB", trim_inv_start, trim_inv_end, outdir+"/cinv_cLSTF_"+str(num)+".DDB")
...
def SSTF(subject, averaging_times, angle, trim_imp_start, trim_imp_end, subdir):
    ...
    if str(angle)=="check":                                                                      #追加した行
        for LR in ['L', 'R']:
            ...
            trim(outdir+"/SSTF_check_"+LR+".DDA", trim_imp_start, trim_imp_end, outdir+"/cSSTF_check_"+LR+".DDB")
    else:
        for LR in ['L', 'R']:
